...
class HomeController extends GetxController {
  ...
  Future<void> calculateAmount() async {
    try {
      log('Calculating amount: distance=${distance.value}, selectedType=${selectedType.value.title}, kmCharge=${selectedType.value.kmCharge}');

      if (distance.value == "0" || distance.value.isEmpty) {
        amount.value = "";
        log('Distance is 0 or empty, amount not calculated');
        return;
      }

      if (selectedType.value.id == null ||
          selectedType.value.kmCharge == null) {
        amount.value = "";
        log('No service type or kmCharge set, amount not calculated');
        ShowToastDialog.showToast(
            "Please select a service type to calculate the fare.");
        return;
      }
      ...
      if (distanceValue <= 0) {
        amount.value = "";
        log('Invalid distance: $distanceValue');
        return;
      }

      // Calculate amount using Constant.amountCalculate
      amount.value = Constant.amountCalculate(
        selectedType.value.kmCharge.toString(),
        distance.value,
      ).toStringAsFixed(Constant.currencyModel!.decimalDigits!);

      log('Amount calculated: ${amount.value} for distance: $distanceValue');
    } catch (e, stackTrace) {
      log('Error calculating amount: $e, stackTrace: $stackTrace');
      amount.value = "";
      ShowToastDialog.showToast("Failed to calculate amount: $e");
    }
  }
...
Rx<PaymentModel> paymentModel = PaymentModel().obs;
 ...
 getPaymentData() async {
  ...
  GoogleMapController?
      mapController; // Add this line to define the mapController property
}
